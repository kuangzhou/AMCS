function [I,test_targets, err,prob,Crate]= C4_5(train_patterns, train_targets, test_patterns, ts_label,cn)
...
%     Ub1=unique(test_patterns(i,:));
%     Nb1=length(Ub1);
    Nb = length(Ub);
    if (round(train_patterns(i,:))==train_patterns(i,:) |  Nb <= Nu ),
%         | round(train_patterns(i,:))==train_patterns(i,:)
...
if (discrete_dim(dim) == 0)
    %Continuous pattern
  patterns(dim, indices)
    tree.split_loc
     if isempty(find(patterns(dim, indices) <= tree.split_loc))
         in=indices(randi([1 size(indices,2)]));
     else
